...
void	Purva_Phalguni_By_Manasi(HDC, DWORD, DWORD, DWORD, DWORD);		/********		CHANGE THE FUNCTION NAME HERE		**********/
	...
	switch(iMsg)
	{
		...
		case WM_SIZE:	//	lParam 	->		LOWORD		->	mainwindow_width
			...
			for( int i = 0 ; i < NO_OF_WINDOWS ; i++)
			{
				...
				if( i == 0)
				{
					reference_window[i].fun_ptr_draw = Purva_Phalguni_By_Manasi;		/********		CHANGE THE FUNCTION NAME HERE		**********/
...
void Purva_Phalguni_By_Manasi(HDC hdc, DWORD reference_window_x_pos, DWORD reference_window_y_pos, DWORD reference_window_width, DWORD reference_window_height)
{
	...
	POINT point[9];
	POINT line_point_1[7];
	POINT line_point_2[4];
	POINT line_point_3[2];
	

	// Code


	point[0].x = reference_window_x_pos + (reference_window_width / 2) - GetValueFromPercentage(reference_window_width / 2, 45 );
	point[0].y = reference_window_y_pos + (reference_window_height / 2) - GetValueFromPercentage(reference_window_height /2, 50);

	point[1].x = reference_window_x_pos + (reference_window_width / 2) - GetValueFromPercentage(reference_window_width / 2, 90);
	point[1].y = reference_window_y_pos + (reference_window_height / 2) + GetValueFromPercentage(reference_window_height / 2, 55);
	
	point[2].x = reference_window_x_pos + (reference_window_width / 2) - GetValueFromPercentage(reference_window_width / 2, 40);
	point[2].y = reference_window_y_pos + (reference_window_height / 2) + GetValueFromPercentage(reference_window_height / 2, 50);

	point[3].x = reference_window_x_pos + (reference_window_width / 2) + GetValueFromPercentage(reference_window_width / 2, 65);
	point[3].y = reference_window_y_pos + (reference_window_height / 2) + GetValueFromPercentage(reference_window_height / 2, 60);

	point[4].x = reference_window_x_pos + (reference_window_width / 2) + GetValueFromPercentage(reference_window_width / 2, 63);
	point[4].y = reference_window_y_pos + (reference_window_height / 2) - GetValueFromPercentage(reference_window_height / 2, 15);

	point[5].x = reference_window_x_pos + (reference_window_width / 2) + GetValueFromPercentage(reference_window_width / 2, 45);
	point[5].y = reference_window_y_pos + (reference_window_height / 2) - GetValueFromPercentage(reference_window_height / 2, 55);

	point[6].x = reference_window_x_pos + (reference_window_width / 2) + GetValueFromPercentage(reference_window_width / 2, 45);
	point[6].y = reference_window_y_pos + (reference_window_height / 2) - GetValueFromPercentage(reference_window_height / 2, 85);

	point[7].x = reference_window_x_pos + (reference_window_width / 2) + GetValueFromPercentage(reference_window_width / 2, 65);
	point[7].y = reference_window_y_pos + (reference_window_height / 2) - GetValueFromPercentage(reference_window_height / 2, 100);

	point[8].x = reference_window_x_pos + (reference_window_width / 2) + GetValueFromPercentage(reference_window_width / 2, 75);
	point[8].y = reference_window_y_pos + (reference_window_height / 2) - GetValueFromPercentage(reference_window_height / 2, 88);
	...
	line_point_1[6].x = point[0].x;
	line_point_1[6].y = point[0].y;

	// Second set of lines 

	line_point_2[0].x = point[5].x;
	line_point_2[0].y = point[5].y;

	line_point_2[1].x = point[6].x;
	line_point_2[1].y = point[6].y;

	line_point_2[2].x = point[7].x ;
	line_point_2[2].y = point[7].y ;

	line_point_2[3].x = point[8].x;
	line_point_2[3].y = point[8].y;

	// Third set of lines

	line_point_3[0].x = point[0].x;
	line_point_3[0].y = point[0].y;

	line_point_3[1].x = point[2].x;
	line_point_3[1].y = point[2].y;
	...
	Polyline(hdc, line_point_1, 7);
	Polyline(hdc, line_point_2, 4);
	Polyline(hdc, line_point_3, 2);
