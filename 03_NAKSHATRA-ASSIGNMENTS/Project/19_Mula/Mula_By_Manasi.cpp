...
#define			CIRCLE_RADIUS_PERCENT	0.8			/*******		CHANGE THE PERCENT FOR RADIUS HERE		*******/
...
void	Mula_By_Manasi(HDC, DWORD, DWORD, DWORD, DWORD);		/********		CHANGE THE FUNCTION NAME HERE		**********/
	...
	switch(iMsg)
	{
		...
		case WM_SIZE:	//	lParam 	->		LOWORD		->	mainwindow_width
			...
			for( int i = 0 ; i < NO_OF_WINDOWS ; i++)
			{
				...
				if( i == 0)
				{
					reference_window[i].fun_ptr_draw = Mula_By_Manasi;		/********		CHANGE THE FUNCTION NAME HERE		**********/
...
void Mula_By_Manasi(HDC hdc, DWORD reference_window_x_pos, DWORD reference_window_y_pos, DWORD reference_window_width, DWORD reference_window_height)
{
	...
	point[0].x = reference_window_x_pos + (reference_window_width / 2) - GetValueFromPercentage(reference_window_width / 2, 70 );
	point[0].y = reference_window_y_pos + (reference_window_height / 2) + GetValueFromPercentage(reference_window_height /2, 10);

	point[1].x = reference_window_x_pos + (reference_window_width / 2) - GetValueFromPercentage(reference_window_width / 2, 60);
	point[1].y = reference_window_y_pos + (reference_window_height / 2) + GetValueFromPercentage(reference_window_height / 2, 11);
	
	point[2].x = reference_window_x_pos + (reference_window_width / 2) - GetValueFromPercentage(reference_window_width / 2, 53);
	point[2].y = reference_window_y_pos + (reference_window_height / 2) + GetValueFromPercentage(reference_window_height / 2, 13);

	point[3].x = reference_window_x_pos + (reference_window_width / 2) - GetValueFromPercentage(reference_window_width / 2, 63);
	point[3].y = reference_window_y_pos + (reference_window_height / 2) + GetValueFromPercentage(reference_window_height / 2, 30);

	point[4].x = reference_window_x_pos + (reference_window_width / 2) - GetValueFromPercentage(reference_window_width / 2, 70);
	point[4].y = reference_window_y_pos + (reference_window_height / 2) + GetValueFromPercentage(reference_window_height / 2, 35);

	point[5].x = reference_window_x_pos + (reference_window_width / 2) - GetValueFromPercentage(reference_window_width / 2, 80);
	point[5].y = reference_window_y_pos + (reference_window_height / 2) + GetValueFromPercentage(reference_window_height / 2, 50);

	point[6].x = reference_window_x_pos + (reference_window_width / 2) - GetValueFromPercentage(reference_window_width / 2, 67);
	point[6].y = reference_window_y_pos + (reference_window_height / 2) + GetValueFromPercentage(reference_window_height / 2, 75);
	...
	point[8].x = reference_window_x_pos + (reference_window_width / 2) - GetValueFromPercentage(reference_window_width / 2, 25);
	point[8].y = reference_window_y_pos + (reference_window_height / 2) + GetValueFromPercentage(reference_window_height / 2, 80);

	point[9].x = reference_window_x_pos + (reference_window_width / 2) - GetValueFromPercentage(reference_window_width / 2, 15);
	point[9].y = reference_window_y_pos + (reference_window_height / 2) + GetValueFromPercentage(reference_window_height / 2, 35);

	point[10].x = reference_window_x_pos + (reference_window_width / 2) - GetValueFromPercentage(reference_window_width / 2, 5);
	point[10].y = reference_window_y_pos + (reference_window_height / 2) + GetValueFromPercentage(reference_window_height / 2, 15);

	point[11].x = reference_window_x_pos + (reference_window_width / 2) + GetValueFromPercentage(reference_window_width / 2, 20);
	point[11].y = reference_window_y_pos + (reference_window_height / 2) - GetValueFromPercentage(reference_window_height / 2, 10);

	point[12].x = reference_window_x_pos + (reference_window_width / 2) + GetValueFromPercentage(reference_window_width / 2, 30);
	point[12].y = reference_window_y_pos + (reference_window_height / 2) - GetValueFromPercentage(reference_window_height / 2, 20);

	point[13].x = reference_window_x_pos + (reference_window_width / 2) + GetValueFromPercentage(reference_window_width / 2, 45);
	point[13].y = reference_window_y_pos + (reference_window_height / 2) - GetValueFromPercentage(reference_window_height / 2, 18);

	point[14].x = reference_window_x_pos + (reference_window_width / 2) + GetValueFromPercentage(reference_window_width / 2, 55);
	point[14].y = reference_window_y_pos + (reference_window_height / 2) - GetValueFromPercentage(reference_window_height / 2, 60);

	point[15].x = reference_window_x_pos + (reference_window_width / 2) + GetValueFromPercentage(reference_window_width / 2, 45);
	point[15].y = reference_window_y_pos + (reference_window_height / 2) - GetValueFromPercentage(reference_window_height / 2, 5);

	point[16].x = reference_window_x_pos + (reference_window_width / 2) + GetValueFromPercentage(reference_window_width / 2, 65);
	point[16].y = reference_window_y_pos + (reference_window_height / 2) - GetValueFromPercentage(reference_window_height / 2, 55);

	point[17].x = reference_window_x_pos + (reference_window_width / 2) + GetValueFromPercentage(reference_window_width / 2, 65);
	point[17].y = reference_window_y_pos + (reference_window_height / 2) + GetValueFromPercentage(reference_window_height / 2, 10);

	point[18].x = reference_window_x_pos + (reference_window_width / 2) + GetValueFromPercentage(reference_window_width / 2, 70);
	point[18].y = reference_window_y_pos + (reference_window_height / 2) - GetValueFromPercentage(reference_window_height / 2, 30);

	point[19].x = reference_window_x_pos + (reference_window_width / 2) + GetValueFromPercentage(reference_window_width / 2, 67);
	point[19].y = reference_window_y_pos + (reference_window_height / 2) - GetValueFromPercentage(reference_window_height / 2, 5);
	...
	line_point_1[4].x = point[4].x;
	line_point_1[4].y = point[4].y;

	line_point_1[5].x = point[5].x;
	line_point_1[5].y = point[5].y;

	line_point_1[6].x = point[6].x;
	line_point_1[6].y = point[6].y;

	line_point_1[7].x = point[7].x;
	line_point_1[7].y = point[7].y;

	line_point_1[8].x = point[8].x;
	line_point_1[8].y = point[8].y;

	line_point_1[9].x = point[9].x;
	line_point_1[9].y = point[9].y;
	
	line_point_1[10].x = point[10].x;
	line_point_1[10].y = point[10].y;

	line_point_1[11].x = point[11].x;
	line_point_1[11].y = point[11].y;

	line_point_1[12].x = point[12].x;
	line_point_1[12].y = point[12].y;

	line_point_1[13].x = point[13].x;
	line_point_1[13].y = point[13].y;


	// Second set of lines 
	
	line_point_2[0].x = point[13].x;
	line_point_2[0].y = point[13].y;

	line_point_2[1].x = point[14].x;
	line_point_2[1].y = point[14].y;

	line_point_2[2].x = point[16].x;
	line_point_2[2].y = point[16].y;

	line_point_2[3].x = point[18].x;
	line_point_2[3].y = point[18].y;

	// Third set of lines
	
	line_point_3[0].x = point[13].x;
	line_point_3[0].y = point[13].y;

	line_point_3[1].x = point[15].x;
	line_point_3[1].y = point[15].y;

	line_point_3[2].x = point[17].x;
	line_point_3[2].y = point[17].y;

	line_point_3[3].x = point[19].x;
	line_point_3[3].y = point[19].y;
	...
	Polyline(hdc, line_point_1, 14);
	Polyline(hdc, line_point_2, 4);
	...
	//	Red Dot representing Nakshatra
	...
	DrawCircle(hdc, point[19].x, point[19].y, circle_radius);
