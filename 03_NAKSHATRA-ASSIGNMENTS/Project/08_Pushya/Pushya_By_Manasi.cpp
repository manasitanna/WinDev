...
void	Pushya_By_Manasi(HDC, DWORD, DWORD, DWORD, DWORD);		/********		CHANGE THE FUNCTION NAME HERE		**********/
	...
	switch(iMsg)
	{
		...
		case WM_SIZE:	//	lParam 	->		LOWORD		->	mainwindow_width
			...
			for( int i = 0 ; i < NO_OF_WINDOWS ; i++)
			{
				...
				if( i == 0)
				{
					reference_window[i].fun_ptr_draw = Pushya_By_Manasi;		/********		CHANGE THE FUNCTION NAME HERE		**********/
...
void Pushya_By_Manasi(HDC hdc, DWORD reference_window_x_pos, DWORD reference_window_y_pos, DWORD reference_window_width, DWORD reference_window_height)
{
	...
	POINT point[6];
	POINT line_point_1[4];

	// Code
	point[0].x = reference_window_x_pos + (reference_window_width / 2) - GetValueFromPercentage((reference_window_width / 2), 53);
	...
	point[1].y = reference_window_y_pos + (reference_window_height / 2) - GetValueFromPercentage((reference_window_height / 2), 23);

	point[2].x = reference_window_x_pos + (reference_window_width / 2) + GetValueFromPercentage((reference_window_width / 2), 10);
	point[2].y = reference_window_y_pos + (reference_window_height / 2) - GetValueFromPercentage((reference_window_height / 2), 55);

	point[3].x = reference_window_x_pos + (reference_window_width / 2) + GetValueFromPercentage((reference_window_width / 2), 30);
	point[3].y = reference_window_y_pos + (reference_window_height / 2) - GetValueFromPercentage((reference_window_height / 2), 100);

	point[4].x = reference_window_x_pos + (reference_window_width / 2) - GetValueFromPercentage((reference_window_width / 2), 30);
	point[4].y = reference_window_y_pos + (reference_window_height / 2) + GetValueFromPercentage((reference_window_height / 2), 100);

	point[5].x = reference_window_x_pos + (reference_window_width / 2) + GetValueFromPercentage((reference_window_width / 2), 20);
	point[5].y = reference_window_y_pos + (reference_window_height / 2) + GetValueFromPercentage((reference_window_height / 2), 50);

	// Connecting Lines

	line_point_1[0].x = point[2].x;
	line_point_1[0].y = point[2].y;

	line_point_1[1].x = point[5].x;
	line_point_1[1].y = point[5].y;

	line_point_1[2].x = point[4].x;
	line_point_1[2].y = point[4].y;

	line_point_1[3].x = point[1].x;
	line_point_1[3].y = point[1].y;
